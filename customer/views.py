...
import requests
from django.shortcuts import render
from .forms import ClaimFormForm
...
def claim_form(request):
    if request.method == 'POST':
        ...
        form = ClaimFormForm()
    return render(request, 'customer/claim.html', {'form': form})


# def progess_view(request):
# return render(request, 'customer/progess.html')
def index(request):
    if request.method == 'POST':
        cl = MpesaClient()
        # Use a Safaricom phone number that you have access to, for you to be able to view the prompt.
        phone_number = '0716900335'
        amount = 1
